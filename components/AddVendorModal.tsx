...
export const AddVendorModal: React.FC<AddVendorModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const [formData, setFormData] = useState<Omit<Vendor, 'mesesElegibilidade'>>(initialFormState);
  ...
  useEffect(() => {
    const checkCodigo = async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/parceiros/check-codigo?codigo=${debouncedCod}`);
            if (response.ok) {
                ...
                if (data.exists) {
                 ...
                 setCodCheck({ status: 'exists', message: '⚠️ Erro ao verificar código' });
            }
        } catch (error) {
            console.error("API call failed:", error);
            // In a real app, handle this error more gracefully.
            // For now, assume it exists to prevent duplicates on error.
            setCodCheck({ status: 'exists', message: '⚠️ Erro de rede ao verificar' });
        }
    };
    checkCodigo();
  }, [debouncedCod]);
  ...
  const isFormInvalid = !formData.cod || codCheck.status !== 'available' || !formData.nome || !formData.cpfCnpj || !!cpfCnpjError || !formData.pix;
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
            ...
            <div className="p-6 bg-gray-50 flex justify-end space-x-3">
                <button type="button" onClick={handleClose} className="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                    Cancelar
                </button>
